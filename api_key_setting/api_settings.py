...
# 導入API金鑰管理模組
from api_key_setting.test_api_connect import (
    test_api_connection,
)
from api_key_setting.api_key_manager import (
    save_api_key_permanently,
)
...
class ApiSettingsDialog:
    ...
    def show(self):
        ...
        ttk.Checkbutton(
            api_frame, text="顯示", variable=self.show_var, command=toggle_show
        ).pack(side=tk.LEFT, padx=5)

        # 添加說明標籤
        ttk.Label(
            frame,
            text="請輸入您的Gemini API密鑰。您可以從Google AI Studio獲取免費的API密鑰。",
            wraplength=480,
        ...
        note_label = ttk.Label(
            frame,
            text=f"注意：API密鑰將僅保存在當前程序會話中，程序關閉後將失效。\n要永久保存API密鑰，請使用下方的「永久保存」按鈕或設置系統環境變量 {Config.API_KEY_ENV_NAME}。",
            foreground="red",
            wraplength=480,
        )
        ...
        ttk.Button(test_frame, text="測試連接", command=self.test_connection).pack(
            side=tk.LEFT, padx=5
        )
        ...
        # 添加保存按鈕
        ttk.Button(btn_frame, text="保存到會話", command=self.save_and_close).pack(
            side=tk.LEFT, padx=5
        )

        # 添加永久保存按鈕
        ttk.Button(btn_frame, text="永久保存", command=self.save_permanently).pack(
            pady=5, fill=tk.X
        )
    ...
    def save_permanently(self):
        ...
        confirm = messagebox.askyesno(
            "確認永久保存",
            f"此操作將嘗試將API密鑰永久保存到系統環境變數中 ({Config.API_KEY_ENV_NAME})。\n\n"
            "這可能需要管理員權限，並且會修改您的系統配置。\n"
            "您確定要繼續嗎？",
            parent=self.dialog,
        )
        ...
        if success:
            ...
            messagebox.showerror(
                "保存失敗",
                f"無法永久保存API密鑰：\n{message}\n\n"
                f"請嘗試手動設置系統環境變數 {Config.API_KEY_ENV_NAME}。",
                parent=self.dialog,
            )
