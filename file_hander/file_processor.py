    ...
    def process_files(
        ...
        try:
            ...
            for line in self.process.stdout:
                ...
                # 檢查是否找到了文件總數 - 同時支持中英文
                if (
                    ("Found" in line and "files" in line)
                    or ("找到" in line and "個Python文件" in line)
                    or ("找到" in line and "個文件" in line)
                ):
                    try:
                        # 嘗試從不同格式的輸出中提取文件數量
                        if "Found" in line and "files" in line:
                            ...
                            total_files = int(
                                line.split("找到")[1].split("個")[0].strip()
                            )
                ...
                # 檢查是否處理了新文件 - 同時支持中英文
                    ...
                    ("Completed" in line and "->" in line)
                    or ("已完成" in line)
                    or ("Processing completed" in line and "successfully" in line)
                ):
                    # 如果是處理完成的總結行，提取總數
                    ...
                    if total_files > 0:
                        self.queue.put(
                            ("status", f"已處理 {processed_count}/{total_files} 個文件")
                        )
                    else:
            ...
            if return_code == 0:
                if total_files > 0:
                    self.queue.put(
                        ("status", f"成功處理了 {processed_count}/{total_files} 個文件")
                    )
                    self.queue.put(
                        (
                            "message",
                            (
                                "處理完成",
                                f"成功處理了 {processed_count}/{total_files} 個文件",
                                False,
                            ),
                        )
                    )
                    ...
                    self.queue.put(
                        (
                            "message",
                            ("處理完成", f"成功處理了 {processed_count} 個文件", False),
                        )
                    )
            else:
                self.queue.put(
                    ("status", f"處理完成，但有錯誤 (返回碼: {return_code})")
                )
                self.queue.put(
                    (
                        "message",
