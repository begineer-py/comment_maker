...
class SettingsPanel:
    ...
    def __init__(self, parent, on_browse_folder, on_browse_output):
        ...
        # 源文件夾設置
        ttk.Label(self.grid, text="源文件夾:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
        ...
        ttk.Button(self.folder_frame, text="瀏覽...", command=self.on_browse_folder).pack(side=tk.RIGHT, padx=5)
        
        # 輸出文件夾設置
        ttk.Label(self.grid, text="輸出文件夾:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
        ...
        ttk.Button(self.output_frame, text="瀏覽...", command=self.on_browse_output).pack(side=tk.RIGHT, padx=5)
        
        # 遞歸處理設置
        ttk.Label(self.grid, text="遞歸處理:").grid(row=2, column=0, sticky=tk.W, padx=5, pady=5)
        
        self.recursive_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(self.grid, variable=self.recursive_var).grid(row=2, column=1, sticky=tk.W, padx=5, pady=5)
        
        # 文件過濾器設置
        ttk.Label(self.grid, text="文件過濾器:").grid(row=3, column=0, sticky=tk.W, padx=5, pady=5)
        ...
        ttk.Label(self.filter_frame, text="(例如: *.py,*.js,*.html)").pack(side=tk.RIGHT, padx=5)
        
        # 註釋風格設置
        ttk.Label(self.grid, text="註釋風格:").grid(row=4, column=0, sticky=tk.W, padx=5, pady=5)
        ...
        ttk.Radiobutton(self.style_frame, text="行尾註釋", variable=self.style_var, value="line_end").pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(self.style_frame, text="行前註釋", variable=self.style_var, value="line_start").pack(side=tk.LEFT, padx=5)
        
        # 模型設置
        ttk.Label(self.grid, text="模型:").grid(row=5, column=0, sticky=tk.W, padx=5, pady=5)
        ...
        self.model_combo["values"] = ("gemini-1.5-pro", "gemini-1.5-flash", "gemini-1.0-pro", "gemini-pro")
        self.model_combo.pack(side=tk.LEFT, fill=tk.X, expand=True)
        
        # 延遲設置
        ttk.Label(self.grid, text="請求延遲(秒):").grid(row=6, column=0, sticky=tk.W, padx=5, pady=5)
        ...
        self.delay_spin = ttk.Spinbox(self.delay_frame, from_=0.5, to=30.0, increment=0.5, textvariable=self.delay_var, width=10)
...
class HelpDialog:
    ...
    def show(self):
        ...
        # 創建標題
        title_label = ttk.Label(frame, text="Gemini代碼註釋器使用幫助", font=("Arial", 14, "bold"))
...
如需更多幫助，請訪問項目主頁或聯繫開發者。
        ...
        # 添加關閉按鈕
        ttk.Button(frame, text="關閉", command=self.dialog.destroy).pack(side=tk.RIGHT, pady=10)
        
        # 添加訪問Google AI Studio的按鈕
        def open_ai_studio():
            webbrowser.open("https://aistudio.google.com/")
        
        ttk.Button(frame, text="訪問Google AI Studio", command=open_ai_studio).pack(side=tk.LEFT, pady=10)
...
class AboutDialog:
    ...
    def show(self):
        ...
        # 創建標題
        title_label = ttk.Label(frame, text="Gemini代碼註釋器", font=("Arial", 16, "bold"))
        ...
        # 創建描述
        desc_label = ttk.Label(frame, text="使用Google Gemini AI為代碼添加中文註釋的工具", wraplength=350)
        ...
        # 創建技術信息
        tech_label = ttk.Label(frame, text="使用Python和Tkinter開發\n基於Google Generative AI API", justify=tk.CENTER)
        tech_label.pack(anchor=tk.CENTER, pady=(0, 20))
        
        # 添加關閉按鈕
        ttk.Button(frame, text="關閉", command=self.dialog.destroy).pack(side=tk.BOTTOM, pady=10) 