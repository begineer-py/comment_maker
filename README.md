...
- `--comment-style`: 註釋風格，line_end（行尾註釋）（默認：line_end）
...
- **API密鑰**：您需要有一個或多個有效的Google Gemini API密鑰，並通過環境變量 `GEMINI_API_KEY` 設置或是更改config.yaml。
- **文件大小**：處理大型文件可能需要較長時間。
...
## 更新日誌
由於行前註釋容易導致縮進問題，本專案已將行前註釋功能刪除，請使用行尾註釋功能。
